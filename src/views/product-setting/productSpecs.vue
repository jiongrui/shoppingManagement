<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('table.spec')" min-width="150px" align="center">
        <template slot-scope="scope">
    ...
    <el-dialog :title="$t(textMap[dialogStatus])" :visible.sync="dialogFormVisible">
      ...
      >
        <el-form-item :label="$t('table.spec')" prop="name">
...
<script>
import {
  fetchProductSpecList,
  createProductSpec,
  updateProductSpec,
  deleteProductSpec
} from "@/api/productSpecs";
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchProductSpecList(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          createProductSpec(this.temp).then(() => {
    ...
    handleDelete(row) {
      this.$confirm("确定删除?", "提示", {
        ...
        .then(() => {
          deleteProductSpec({ _id: row._id }).then(res => {
            this.$message({
              type: "success",
              message: "删除成功"
            });
