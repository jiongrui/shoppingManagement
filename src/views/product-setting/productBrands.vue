<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('table.brand')" min-width="150px" align="center">
        <template slot-scope="scope">
    ...
    <el-dialog :title="$t(textMap[dialogStatus])" :visible.sync="dialogFormVisible">
      ...
      >
        <el-form-item :label="$t('table.brand')" prop="name">
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchProductBrandList(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          createProductBrand(this.temp).then(() => {
    ...
    updateData() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          ...
          updateProductBrand(this.temp).then(() => {
            this.dialogFormVisible = false;
            this.$message({
              type: "success",
              message: "更新成功"
    ...
    handleDelete(row) {
      this.$confirm("确定删除?", "提示", {
        ...
        .then(() => {
          deleteProductBrand({ _id: row._id }).then(res => {
            this.$message({
              type: "success",
              message: "删除成功"
            });
